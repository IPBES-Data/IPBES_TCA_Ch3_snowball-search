...
snowball$edges <- snowball$edges |>
    mutate(from_kp = from %in% ids, to_kp = to %in% ids)


ggraph(graph = as_tbl_graph(snowball), layout = "stress") +
    geom_edge_link(aes(alpha = after_stat(index)), show.legend = FALSE, arrow = arrow(length = unit(1, 'mm'))) +
...
```{r}
#| label: identify_two_key_paper_sampling
#| 

duplicated(x$from) & !(x$from_kp)
